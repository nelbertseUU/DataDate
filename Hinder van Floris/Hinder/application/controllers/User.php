...
class User extends MY_Controller {
    ...
    public function index(){
        ...
        foreach($this->User_model->getUserById($UserID) as $row){
            ...
            $data['profielfoto'] = $row['ProfielFoto'];
        }
    ...
    */
    ...
    function do_upload()
	{
		...
		if ( ! $this->upload->do_upload())
		{
            ...
            $data['error'] = implode(', ', array('error' => $this->upload->display_errors()));
        ...
        {
            $UserID = $_SESSION['UserID'];
            $path = 'uploads/thumbnails/'.($this->upload->data()['raw_name']).'_thumb'.($this->upload->data()['file_ext']);
            $this->do_resize(($this->upload->data()['file_name']));
            $this->User_model->insertImg($path, $UserID);
            redirect('User');
		}
	}  
