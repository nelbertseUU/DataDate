...
class Match_model extends CI_Model{
    ...
    public function getUserByPersVk(){
        $sql = "SELECT UserID, TYPE FROM Persoonlijkheidstype";
        $query = $this->db->query($sql, array());
        return $query->result_array();
    }
    
     public function getAB(){
        $sql = "SELECT alfa, beta, xfactor FROM AlfaBeta";
         $query = $this->db->query($sql, array());
        return $query->result_array();
     }
    public function updateWaarde($vk, $nieuw, $id){
        $sql = "UPDATE Voorkeuren SET ? = ? WHERE UserID = ?";
        $this->db->query($sql, array($vk, $nieuw, $id));
        
    }
    public function UpdateAB($a, $b){
        $sql = "UPDATE AlfaBeta SET alfa = ?, beta = ?";
            $this->db->query($sql, array($a, $b));
    }
     public function UpdateX($a){
        $sql = "UPDATE AlfaBeta SET xfactor = ?";
            $this->db->query($sql, array($a));
    }
}
?>